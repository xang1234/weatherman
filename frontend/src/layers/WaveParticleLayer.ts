/**
 * GPU wave-particle layer with trail rendering.
 *
 * Mirrors WindParticleLayer but driven by wave height + direction data
 * instead of wind U/V components. Particles flow in the wave propagation
 * direction with speed proportional to significant wave height.
 *
 * Three-pass pipeline per frame (identical structure to wind):
 *   Pass 1 — State Update (ping-pong via wave-particle-update shader)
 *   Pass 2 — Trail Composite (reuses trail-composite shader)
 *   Pass 3 — Map Composite (reuses trail-composite shader)
 */
...
/** Maximum expected wave height (m) for normalizing height → alpha in the draw shader. */
...
export class WaveParticleLayer implements CustomLayerInterface {
  ...
  // ── Wave data tile managers ─────────────────────────────────────────
  private _waveHeightManager: TileManager | null = null
  private _waveDirManager: TileManager | null = null
  private _waveHeightT1Manager: TileManager | null = null
  private _waveDirT1Manager: TileManager | null = null
  ...
  private _uUpdateWaveDirection: WebGLUniformLocation | null = null
  private _uUpdateWaveDirectionT1: WebGLUniformLocation | null = null
  ...
  private _atlasDir: WebGLTexture | null = null
  private _atlasHeightT1: WebGLTexture | null = null
  private _atlasDirT1: WebGLTexture | null = null
  ...
  render(
    ...
    // ── Update wave tile managers with current viewport ──
    ...
    const atlas = this._waveConfigured
    ...
    // Bind wave height atlas to units 1-2 (T0, T1)
    ...
    // Bind wave direction atlas to units 3-4 (T0, T1)
    gl.activeTexture(gl.TEXTURE3)
    gl.bindTexture(gl.TEXTURE_2D, atlas ? this._atlasDir : null)
    gl.uniform1i(this._uUpdateWaveDirection, 3)

    gl.activeTexture(gl.TEXTURE4)
    gl.bindTexture(gl.TEXTURE_2D, atlas && hasT1 ? this._atlasDirT1 : null)
    gl.uniform1i(this._uUpdateWaveDirectionT1, 4)
    ...
    // Wave height range [0, 15] and direction range [0, 360]
    gl.uniform1f(this._uUpdateValueMinHeight, 0)
    gl.uniform1f(this._uUpdateValueMaxHeight, 15)
    gl.uniform1f(this._uUpdateValueMinDir, 0)
    gl.uniform1f(this._uUpdateValueMaxDir, 360)
    // Zoom-adaptive speed scale: ensure particles move TARGET_DISP_PX per frame for REF_WAVE_HEIGHT_M.
    const safeDt = Math.max(dt, 0.004)
    const speedScale = TARGET_DISP_PX / (REF_WAVE_HEIGHT_M * safeDt * worldSize)
    ...
    // Request next frame only when animating (wave data active or mid-transition)
  ...
  /** Configure the wave dataset to fetch tiles from. */
  setWaveConfig(model: string, runId: string, forecastHour: number): void {
    ...
    this._waveDirManager?.setLayer(model, runId, 'wave_direction', forecastHour)
    this._waveConfigured = true
    this._map?.triggerRepaint()
  }

  /** Set temporal interpolation for the particle wave field. */
  setTemporalBlend(forecastHourT1: number, mix: number): void {
    ...
    if (forecastHourT1 >= 0 && this._model && this._runId) {
      ...
      this._waveDirT1Manager?.setLayer(this._model, this._runId, 'wave_direction', forecastHourT1)
    }
    this._map?.triggerRepaint()
  }
  ...
  advanceForecastHour(newHour: number): void {
    if (
      ...
      this._waveDirT1Manager &&
      this._waveDirT1Manager.cacheSize > 0 &&
      this._waveDirT1Manager.currentForecastHour === newHour
    ) {
      ...
      ;[this._waveDirManager, this._waveDirT1Manager] = [this._waveDirT1Manager, this._waveDirManager]
    } else {
      // T1 tiles not ready — reconfigure T0 for the new hour (starts fresh fetch)
      console.debug(
        `[WaveParticleLayer] advanceForecastHour: T1 tiles not ready for hour ${newHour}, reconfiguring T0`
      )
      if (this._model && this._runId) {
        this._waveHeightManager?.setLayer(this._model, this._runId, 'wave_height', newHour)
        this._waveDirManager?.setLayer(this._model, this._runId, 'wave_direction', newHour)
      }
    }
    this._temporalMix = 0
    this._map?.triggerRepaint()
  ...
  isT1Ready(): boolean {
    return (
      ...
      (this._waveDirT1Manager?.cacheSize ?? 0) > 0
    )
  }
  ...
  /** Create tile managers for wave data fetching. */
  private _initTileManagers(gl: WebGL2RenderingContext): void {
    ...
    this._waveDirManager = new TileManager(gl, tmOpts)
    this._waveDirManager.onTileLoaded = triggerRepaint
    this._waveHeightT1Manager = new TileManager(gl, tmOpts)
    this._waveHeightT1Manager.onTileLoaded = triggerRepaint
    this._waveDirT1Manager = new TileManager(gl, tmOpts)
    this._waveDirT1Manager.onTileLoaded = triggerRepaint
  }
  ...
  private _initResources(gl: WebGL2RenderingContext): void {
    ...
    this._uUpdateWaveDirection = gl.getUniformLocation(up, 'u_waveDirection')
    this._uUpdateWaveDirectionT1 = gl.getUniformLocation(up, 'u_waveDirectionT1')
  ...
  private _packAtlas(
    ...
    for (const c of visibleCoords) {
      ...
      const dirTex = this._waveDirManager?.getTexture(c.z, c.x, c.y) ?? null
      if (heightTex && this._atlasWaveH) {
        this._copyTileToAtlas(gl, heightTex, this._atlasWaveH, col, row)
        hasAnyTile = true
      }
      if (dirTex && this._atlasDir) {
        this._copyTileToAtlas(gl, dirTex, this._atlasDir, col, row)
      }
    }
    ...
    if (this._forecastHourT1 >= 0 && this._temporalMix > 0) {
      for (const c of visibleCoords) {
        ...
        const col = rx - minRX
        const row = c.y - minY
        const hT1 = this._waveHeightT1Manager?.getTexture(c.z, c.x, c.y) ?? null
        const dT1 = this._waveDirT1Manager?.getTexture(c.z, c.x, c.y) ?? null
        if (hT1 && this._atlasHeightT1) this._copyTileToAtlas(gl, hT1, this._atlasHeightT1, col, row)
        if (dT1 && this._atlasDirT1) this._copyTileToAtlas(gl, dT1, this._atlasDirT1, col, row)
      }
    }
  ...
  private _ensureAtlas(gl: WebGL2RenderingContext, cols: number, rows: number): void {
    ...
    this._atlasWaveH = this._createAtlasTexture(gl, w, h)
    this._atlasDir = this._createAtlasTexture(gl, w, h)
    this._atlasHeightT1 = this._createAtlasTexture(gl, w, h)
    this._atlasDirT1 = this._createAtlasTexture(gl, w, h)
  }
  ...
  private _clearAtlas(gl: WebGL2RenderingContext): void {
    const atlases = [this._atlasWaveH, this._atlasDir, this._atlasHeightT1, this._atlasDirT1]
  ...
  private _deleteAtlasTextures(gl: WebGL2RenderingContext): void {
    ...
    if (this._atlasDir) { gl.deleteTexture(this._atlasDir); this._atlasDir = null }
    if (this._atlasHeightT1) { gl.deleteTexture(this._atlasHeightT1); this._atlasHeightT1 = null }
    if (this._atlasDirT1) { gl.deleteTexture(this._atlasDirT1); this._atlasDirT1 = null }
    this._atlasWidth = 0
    this._atlasHeight = 0
  }
  ...
  private _cleanup(): void {
    ...
    this._waveDirManager?.destroy()
    this._waveDirManager = null
    this._waveHeightT1Manager?.destroy()
    this._waveHeightT1Manager = null
    this._waveDirT1Manager?.destroy()
    this._waveDirT1Manager = null
    ...
    this._uUpdateWaveDirection = null
    this._uUpdateWaveDirectionT1 = null
